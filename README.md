...
Dataset utama yang digunakan adalah Financial Dataset. Pada dataset tersebut sudah tersedian `Investasi_Cost`. Untuk nilai pengurangan emisi, data diambil dari Environmental Dataset. Untuk mencari nilai $CF_t$ dan $r$, kita menggunakan asumsi bahwa $C_t$ adalah `Revenue_Stream` dan $r$ adalah suku bunga atau BI rate.
